...
public class JFXTextAreaSkin extends TextAreaSkin {
    ...
    public JFXTextAreaSkin(JFXTextArea textArea) {
        ...
        registerChangeListener2(textArea.disableAnimationProperty(), "DISABLE_ANIMATION", () -> {
    ...
    protected void layoutChildren(final double x, final double y, final double w, final double h) {
        ...
        if (transition == null || transition.getStatus() == Status.STOPPED) {
            if (getSkinnable().isFocused() && ((JFXTextArea) getSkinnable()).isLabelFloat()) {
                setPromptTextFill2(((JFXTextArea) getSkinnable()).getFocusColor());
            }
        }
        ...
        if (((JFXTextArea) getSkinnable()).isLabelFloat()) {
            if (promptText == null) {
                ...
                if (((Region) scrollPane.getContent()).getChildrenUnmodifiable().get(0) instanceof Text) {
                    ...
                    createPromptNode();
                    ReflectionHelper.setFieldContent(TextAreaSkin.class.getSuperclass(), this, "promptNode", promptText);
